...
fn set_velocities(
    ...
    for (shadow, holding, actor) in q_actor.iter_mut() {
        let prop = holding.0;
        let Ok((mut velocity, mut angvel, transform)) = q_prop.get_mut(prop) else {
            error!("Prop entity was deleted or in an invalid state. Ignoring.");
            continue;
        };
        let transform = transform.compute_transform();

        let delta_position = shadow.target_position - transform.translation;

        let delta_rotation = shadow.target_rotation * transform.rotation.inverse();
